...
return new class extends Migration
{
    public function up(): void
    {
            Schema::create('orders', function (Blueprint $table): void {
                ...
                $table->integer('released_by_user_id')->nullable();
                $table->string('release_mode', 32)->nullable();
                $table->text('release_note')->nullable();
                $table->dateTime('created_at')->useCurrent();
                $table->dateTime('updated_at')->useCurrent();
                $table->index(['location_id'], 'idx_orders_location_id');
                $table->index(['order_type'], 'idx_orders_type');
                $table->index(['status'], 'idx_orders_status');
                $table->index(['destination_location_id'], 'idx_orders_destination_location_id');
